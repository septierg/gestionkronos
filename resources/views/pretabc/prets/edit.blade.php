        ...
        <div class="row page-titles" style="position: relative;top: -43px; ">
            ...
            <div class="col-md-7 align-self-center text-right">
                <div class="d-flex justify-content-end align-items-center">
                    ...
                    {!! Form::model($prets, ['method' => 'PATCH','action' => ['PretabcPretController@update', $prets->id],'files' =>true]) !!}
                    <a href="{{ route('membre.edit', $prets->id_demandeur) }}">
        ...
        <div class="row" style="background-color:white">
            <div class="col-12" style="padding-top:10px;">
                ...
                <h3 style="padding-bottom:15px;">Informations de la demande</h3>
                    ...
                    <img src="../../images/icon_pretabc.png" style="width:50px; margin:10px;" alt="kreditpret" class=""><br><br>

                    <label>No d'entente</label>
                    <input type="text" id="id" name="id" disabled value="{{$prets->id}}"><br>

                    <label>Date inscription au prêt</label>
                    <input type="text" id="id" name="id" disabled value="{{$prets->date_creation}}"><br>

                    {!! Form::label('title', 'État',['class'=> 'margin']) !!}
                    ...
                    {!! Form::label('title', 'Montrer le contrat au client',['class'=> 'margin']) !!}
                    ...
                    <select id="ddlStatus" name="responsable" class="form-control">
                        <option value=304 <?php if($prets->responsable == 304) echo"selected"; ?>>Alicia</option>
                        <option value=3305 <?php if($prets->responsable == 3305) echo"selected"; ?>>Félixe</option>
                        <option value=1617 <?php if($prets->responsable == 1617) echo"selected"; ?>>Khalida</option>
                        <option value=1 <?php if($prets->responsable == 1) echo"selected"; ?>>Maxime</option>
                        <option value=63 <?php if($prets->responsable == 63) echo"selected"; ?>>Alexandre</option>
                        <option value=3306 <?php if($prets->responsable == 3306) echo"selected"; ?>>Nancy</option>
                        <option value=3310 <?php if($prets->responsable == 3310) echo"selected"; ?>>Francois</option>
                        <option value=5239 <?php if($prets->responsable == 5239) echo"selected"; ?>>Roxannne</option>
                        <option value=5636 <?php if($prets->responsable == 5636) echo"selected"; ?>>Mathieu</option>
                        <option value=6134 <?php if($prets->responsable == 6134) echo"selected"; ?>>Émilie</option>
                        <option value=6774 <?php if($prets->responsable == 6774) echo"selected"; ?>>Patrick</option>
                        <option value=9062 <?php if($prets->responsable == 9062) echo"selected"; ?>>Math L</option>
                        <option value=9063 <?php if($prets->responsable == 9063) echo"selected"; ?>>Marc-Antoine</option>
                        <option value=9398 <?php if($prets->responsable == 9398) echo"selected"; ?>>Roseline</option>
                        <option value=9399 <?php if($prets->responsable == 9399) echo"selected"; ?>>Emily S</option>
                    </select><br>
                    <br>

                    {!! Form::label('title', 'Statut',['class'=> 'margin']) !!}
                ...
                {!! Form::label('title', 'Montant',['class'=> 'margin']) !!}
                {!! Form::text('montant', null, array_merge(['class' => 'padding_left']))!!}<br>


                {!! Form::label('title', 'Fréquence des remboursements',['class'=> 'margin']) !!}
                <input type="text" id="frequence_remboursement" name="frequence_remboursement" disabled value="{{$prets->frequence_remboursement}}"><br>

                {!! Form::label('title', 'Faillite au cours des 3 prochains mois',['class'=> 'margin']) !!}
                <input type="text" id="faillite" name="faillite" disabled value="{{$prets->faillite}}"><br>


                {!! Form::label('title', 'Date prochaine paie',['class'=> 'margin']) !!}
                <input type="text" id="date_prochaine_paie" name="date_prochaine_paie" disabled value="{{$prets->date_prochaine_paie}}"><br>
                </div>
            </div>
        </div>
        </div>
        <div class="container-fluid" style="padding-bottom:15px;">
            <div class="row" style="background-color:white">
                <div class="col-12" style="padding-top:10px;">
                    ...
                    {!! Form::label('file', 'Création des contrats :') !!}
                    {!! Form::file('file', null )!!}<br><br>

                        {!! Form::label('title', 'Date importation des données',['class'=> 'margin']) !!}
